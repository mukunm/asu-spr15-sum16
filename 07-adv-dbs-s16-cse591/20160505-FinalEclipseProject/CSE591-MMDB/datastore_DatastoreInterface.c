...
#if 0 /* MDB */
	printf("DataStore: Writing - %d\n", g_u4_current_idx);
#endif /* MDB */

	//pu1_curr_tuple = g_pu1_data_store + (g_u4_current_idx * TUPLE_LENGTH);

#if 0 /* MDB */
	printf("DataStore: Writing - %d - Set ID\n", g_u4_current_idx);
	fflush(stdout);
#endif /* MDB */
	set_id_field(g_pu1_curr_tuple, u4_id);
#if 0 /* MDB */
	{
		int i = 0;
		printf("DataStore: Writing - %d - Set Name = ", g_u4_current_idx);
		fflush(stdout);
		for(i = 0; i < 10; i++) {
			printf("%c", pu1_name[i]);
			fflush(stdout);
...
#endif /* MDB */
	set_name_field(g_pu1_curr_tuple, pu1_name);
#if 0 /* MDB */
	printf("DataStore: Writing - %d - Set Age\n", g_u4_current_idx);
	fflush(stdout);
#endif /* MDB */
	set_age_field(g_pu1_curr_tuple, u1_age);
#if 0 /* MDB */
	printf("DataStore: Writing - %d - Set Salary\n", g_u4_current_idx);
	fflush(stdout);
#endif /* MDB */
	set_salary_field(g_pu1_curr_tuple, u4_salary);

#if 0 /* MDB */
	printf("DataStore: Writing - %d - Increment\n", g_u4_current_idx);
	fflush(stdout);
#endif /* MDB */
	g_u4_current_idx++;
	pu1_prev_tuple = g_pu1_curr_tuple;
	g_pu1_curr_tuple += TUPLE_LENGTH;

#if 0 /* MDB */
	printf("DataStore: Writing - %d - Increment End\n", g_u4_current_idx);
	fflush(stdout);
#endif /* MDB */

	return (jint) pu1_prev_tuple;
}
...
Java_datastore_DatastoreInterface_destroy_1data_1store(JNIEnv *env, jobject obj)
{
	...
	g_pu1_curr_tuple = NULL;
	printf("DataStore: Freed\n");
	fflush(stdout);
	return;
}
